...
export class Cell {
  ...
  get PopulationCount() {
    //Return overall population count
    return Math.round(
      this.IncubatedCount +
        this.InfectedCount +
        this.recovered +
  ...
  get InfectedCount() {
    return Math.round(this.infected.reduce((a, b) => a + b, 0));
  }
  get IncubatedCount() {
    return Math.round(this.incubated.reduce((a, b) => a + b, 0));
  }
  get SusceptibleCount() {
    return this.susceptible;
  }
  get RecoveredCount() {
    return Math.round(this.recovered);
  }
  get Index() {
    return this.index_;
  }
  ...
  SimVirusMorbidity(ageDist: number[], ageMort: number[]) {
    for (let i = 0; i < this.infected.length; i++) {
      for (let j = 0; j < ageMort.length; j++) {
        this.infected[i] -= Math.round(
          this.infected[i] * ageDist[j] * ageMort[j]
        );
      }
    }
  }
   ...
   */
  SimInfections(
    probOfCatchingInfection: number,
    incPeriod: number,
    index: number,
    ...
    if (this.PopulationCount > 0) {
      ...
      const percentageInfected =
        (this.InfectedCount + immigrantsInf - this.infAway) /
